// Requires lib/generate-ip.js + <app|get|log|show>
...
window.api = {
    ...
    createHeaders(api) { // requires lib/generate-ip.js + <app>
        const ip = ipv4.generate({ verbose: false })
        const headers = {
            'Accept': '*/*', 'Accept-Encoding': 'gzip, deflate, br, zstd',
            'Connection': 'keep-alive', 'DNT': '1',
            'Origin': app.apis[api].expectedOrigin.url, 'X-Forwarded-For': ip, 'X-Real-IP': ip
        }
        headers.Referer = headers.Origin + '/'
        if (app.apis[api].method == 'POST') Object.assign(headers, {
            'Content-Type': 'application/json',
            'Host': new URL(app.apis[api].endpoints?.completions || app.apis[api].endpoint).hostname,
            'Sec-Fetch-Site': 'same-origin', 'Sec-Fetch-Dest': 'empty', 'Sec-Fetch-Mode': 'cors'
        })
        Object.assign(headers, app.apis[api].expectedOrigin.headers) // API-specific ones
        if (api == 'OpenAI') headers.Authorization = `Bearer ${app.config.openAIkey}`
        return headers
    },

    createReqData({ api, msgs }) { // returns payload for POST / query string for GET // requires log
    ...
    pick(caller) { // requires <app|get|log>
        log.caller = `get.${caller.name}() » api.pick()`
        const untriedAPIs = Object.keys(app.apis).filter(api =>
            !caller.triedAPIs.some(entry => // exclude tried APIs
                ...
                && ( caller == get.related || ( // handle get.reply exclusions
                    ...
                    && ( // exclude unstreamable APIs if !app.config.streamingDisabled
                        app.config.streamingDisabled || app.apis[api].streamable)
                    && !( // exclude GET APIs if msg history established while not shuffling
                        app.apis[api].method == 'GET' && get.reply.src != 'shuffle' && app.msgChain.length > 2)
                    && !( // exclude APIs that don't support long prompts while summarizing
                        get.reply.src == 'summarize' && app.apis[api].supportsLongPrompts == false)
                ))
        )
        ...
        log.debug('Endpoint chosen', app.apis[chosenAPI].endpoints?.completions || app.apis[chosenAPI].endpoint)
        return chosenAPI
    },

    process: {
        initFailFlags(api) {
            return app.apis[api].respPatterns?.fail ? new RegExp(app.apis[api].respPatterns.fail) : null },

        stream(resp, { caller, callerAPI }) { // requires <app|env|log|show>
            ...
            function handleChunk({ done, value }, callerAPI) {
                ...
                if (done || respChunk.includes(app.apis[callerAPI].respPatterns?.watermark))
    ...
    tryNew({ caller, reason = 'err' }) {
        ...
        log.error(
            `Error using ${ app.apis[caller.api].endpoints?.completions || app.apis[caller.api].endpoint } due to ${reason}`)
        caller.triedAPIs.push({ [caller.api]: reason })
        if (caller.attemptCnt < Object.keys(app.apis).length -+(caller == get.reply)) {
