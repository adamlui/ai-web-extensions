(async () => {
    ...
    addEventListener('message', event => {
        if (event.origin != location.origin || !event.data?.source?.endsWith('chatgpt-widescreen.user.js')) return
    ...
    chrome.runtime.onMessage.addListener(({ action, options, source }) => {
        ({
            ...
            showAbout: async () => {
                if (!source?.endsWith('background.js')) return
    ...
    window.toggleMode = async (mode, state) => {
        ...
        function deactivateMode(mode) {
            if (mode == 'widescreen') { styles.widescreen.node.remove() ; sync.mode('widescreen') }
                ...
                if (app.config.f11) modals.alert(i18n.getMsg('alert_pressF11'), `${i18n.getMsg('alert_f11reason')}.`)
    ...
    new MutationObserver(async () => {
        ...
        if (app.config.extensionDisabled || app.config[`${env.site}Disabled`] || !app.config.btnsVisible) return
