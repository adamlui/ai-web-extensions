...
// @antifeature              ads
// @noframes
// @downloadURL              https://scriptcat.org/scripts/code/6371/googlegpt.user.js
// @updateURL                https://scriptcat.org/scripts/code/6371/googlegpt.meta.js
...
// Documentation: https://github.com/KudoAI/googlegpt/tree/main/docs
...
(async () => {
    ...
    window.toggle = {
        ...
        anchorMode(state = '', { persist = true } = {}) {
            ...
            if (state == 'on' || !state && !app.config.anchored) {
                if (persist) settings.save('anchored', true)
                else app.config.anchored = true
                ;['sticky', 'wider'].forEach(mode => {
                    if (app.config[`${mode}Sidebar`]) { toggle.sidebar(mode) ; sidebarModeToggled = true }})
            } else {
                if (persist) settings.save('anchored', false)
                else app.config.anchored = false
    ...
    // ANCHOR GoogleGPT in sidebar-less Images|Shopping|Short videos|AI Mode
    if (/udm=(?:2|28|39|50)(?:&|$)/.test(location.search)) {
        toggle.anchorMode('on', { persist: false })
